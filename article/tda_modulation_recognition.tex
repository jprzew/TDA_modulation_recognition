...
(other classes: OOK, AM-SSB-SC, FM, etc...)
	...
	A mathematical foundation  of TDA is homology theory, one of the branches of algebraic topology. We are going to focus on the simplicial homology theory which has the clearest geometric motivation and it is sufficient for our consideration. We first introduce the notion of simplicial complexes, which are a natural environment to work with homology theory. 
	
	\subsubsection{Simplicies and simplicial complexes} 
	A simplex is a generalization of triangle, tetrahedron, etc. Examples of $k$-dimentional simplicies are shown in figure \ref{fig:simplices}. They have many advantages like illustrative geometric interpretation and also there is no problem to equip them with algebraic tools. The formal definition of a simplex is as follows. 
	
	\begin{definition}
	A $k$-simplex is a convex hull spanned by $ k+1 $ affinely independent points $v \in \mathbb{R}^m$ and is denoted by:
	...
	\begin{enumerate}
	\item if a simplex $\sigma$ is in $K$, then all faces of $\sigma$ are also in $K$;
	\item the intersection of two simplices in $K$ is either empty or contained in $K$. 
	\end{enumerate}
	\end{definition}
	
	\subsubsection{Simplicial homology}
	To construct a simplicial homology group we need to endow a simplex with orientation. The orientation of $k$-simplex is given by ordering of the vertices in the simplex $[v_1, v_2, ..., v_{k+1}]$. Two orderings can define the same orientation if and only if they differ by an even permutation, thus there are only two possible orientations of simplex. Each oriented simplex is equal to the negative of the simplex with opposite orientation. Mathematically, it is stated as $[v_1, v_2, v_3] = - [v_2, v_1, v_3].$
	
	In simplicial homology we want to identify cycles and we want to know if a given cycle is bounding a collection of higher dimentional simplices: if it does not, then the cycle is a hole. The concept of a simplicial $k$-chain is used in the identification of cycles in a simplicial complex. It is defined as follows. 
	
	\begin{definition}
	A $k$-chain is a finite weighted sum defined on all $k$-simplices
within a complex $K$:
 $$\sum^N_{i=1} c_i \sigma_i, $$

\noindent where $c_i \in \mathbb{Z}$ and $N$ is the number of $k$-dimentional simplices. Typically, the coefficients are given by $c_i \in \{-1, 0, 1\}$. The set of $k$-chains on K is denoted by $C_k(K)$. We use the short-hand notation $C_k(K) = C_k$ for a given $K$.
	\end{definition}
	...
	We use another short-hand notation $\partial(C_k) = \partial_k$ to represent a boundary operation. A set of $k$-chains contains a lot of geometric information about a simplicial complex, however, a large part of it is not useful. We will reduce it by using two subgroups. First of all, we will only consider those chains which are mapped to the null space or kernel of the boundary operator (denoted by $\ker \partial$). 
	...
	\end{definition}
	Second of all, we will identify two cycles if their union is a boundary of a higher dimensional simplex.
	...
	It is also important to note that $B_k$ is a subgroup of $Z_k$. Then we have a difinition of $k$-homology group as follows. 

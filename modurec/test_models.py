...
def make_pipeline(model, features):
    ...
    steps = []
    # selection
    steps.append(('feature_select', FeatureSelector(features)))
    # standarization
    steps.append(('standardize', StandardScaler()))
    # the model
    steps.append(('model', models[model]))
    # create pipeline
...
def test_models(data, features,
    ...
    pipelines = []
    for model in models:
        print("Make predictions on test df for " + model)
        ...
        # disp.plot()
        plot_confusion_matrix(pipe, data_test, y_test)
        plt.show()
